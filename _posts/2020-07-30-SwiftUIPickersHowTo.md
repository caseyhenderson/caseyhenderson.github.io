...
The picker is a UI element of considerable popularity, and also one that has a lot of uses. So how do we use it in SwiftUI? And how do we make sure that our choices are actually saved when the user closes the app? First, we need to consider style.
